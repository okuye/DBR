...
object Main extends zio.App {
  ...
  val routes = Routes.accountRoutes(accountService) <+> Routes.transactionRoutes(accountService)
    ...
    implicit val timer: cats.effect.Timer[Task] = new cats.effect.Timer[Task] {
      override def clock: Clock[Task] = new Clock[Task] {
        override def realTime(unit: TimeUnit): Task[Long] =
          Task.effectTotal(System.currentTimeMillis()).map(unit.convert(_, TimeUnit.MILLISECONDS))

        override def monotonic(unit: TimeUnit): Task[Long] =
          Task.effectTotal(System.nanoTime()).map(unit.convert(_, TimeUnit.NANOSECONDS))
      }
      ...
      override def sleep(duration: FiniteDuration): Task[Unit] =
        zio.clock.sleep(zio.duration.Duration.fromScala(duration)).provideLayer(zio.clock.Clock.live)
